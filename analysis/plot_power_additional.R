#'
#' Plot power curves for JT and DD tests in the additional study
#'
#' This script produces power plots depicted in numerous figures in Part 3 of 
#' the online appendix.
...
additional_symmetric_data_file <- here("data", "additional_symmetric.csv")
additional_asymmetric_data_file <- here("data", "additional_asymmetric.csv")

# Specify output filename, building directory structure if needed
out_dir <- here("outputs", "plots", "power", "additional_study")
if (!dir_exists(out_dir)) { dir_create(out_dir) }


# --- Ingest data ---

if (file.exists(additional_symmetric_data_file)) {
  sym_data <- read.csv(
    additional_symmetric_data_file, 
    header = TRUE,
    strip.white = TRUE
  ) %>% 
    as_tibble() %>%
...
if (file.exists(additional_asymmetric_data_file)) {
  asy_data <- read.csv(
    additional_asymmetric_data_file,
    header = TRUE,
    strip.white = TRUE
  ) %>% 
    as_tibble() %>%
...
# --- Power plots for additional study, symmetric treatment shifts ---
...
# --- Power plots for additional study, asymmetric treatment shifts ---
